...
def get_target_mesh():
	sim = arcsim.getsim()
	arcsim.init_physics('conf/rigidcloth/folds/target1.json',out_path+'/target',False)
	ref = [sim.cloths[0].mesh.nodes[i].x.numpy() for i in sim.cloths[0].mesh.nodes[i]]
...
def get_loss(sim,ref):
	...
	for i in range(ref.shape[0]):
		loss += torch.norm(ref[i]-sim.cloth[0].mesh.nodes[i].x)**2
	loss /= node_number
	loss = loss.to(device)
	return loss
...
if rank != 0:
	...
	torch.save(net.state_dict(),default_path+'/'+task+'_net')
