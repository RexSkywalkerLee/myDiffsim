...
#         sim.cloths[0].mesh.nodes[center].v += net_output.cpu()
        ...
        #sim.cloths[0].mesh.nodes[center].v += param_g[step,:].cpu()
        for i in range(len(handles)):
            sim_input = param_g[step, 3*i:3*i+3]
...
def server_task(optimizer,scheduler,param_g,sim): # This is a loop
    epoch = 0
    global steps, epochs
    while True:
        st = time.time()
        ...
        #net = net.to(device)
        param_g = param_g.to(device)
        #print(next(net.parameters()).grad)
        reset_sim(sim,epoch,rank)
        loss = run_sim(steps,sim,param_g)
        loss.backward()
        #net = net.cpu()
        param_g = param_g.cpu()
        # ====================================
...
if rank != 0:
    ...
    param_g = torch.zeros([steps, 12],dtype=torch.float64, requires_grad=True)
    #net = Net(31,3).cpu()
    optimizer = torch.optim.SGD([{'params':param_g}],lr=10,momentum=0.99)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,10,2,eta_min=0.001)
    sim = arcsim.get_sim()
    reset_sim(sim,rank=rank)
    server_task(optimizer,scheduler,param_g,sim)
    #net = net.to(device)
    param_g = param_g.to(device)
