...
writer = SummaryWriter('rotate_out/exp5')
...
load_torch_model_path = 'rotate_out/exp3/net_weight.pth2020-08-17_23:27:44'
...
with open(out_path+'/log.txt','w',buffering=1) as f:
    ...
    net = Net(48, 12)
    net = net.to(device)
    if os.path.exists(load_torch_model_path):
        net.load_state_dict(torch.load(load_torch_model_path))
        print("load: %s\n success" % load_torch_model_path)
    else:
        print('load fail!')
        quit()
